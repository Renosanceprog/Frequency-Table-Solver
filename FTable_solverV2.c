...
int k, current_lci, current_uci, less_CF=0, great_CF=0, N_total, sum_fx=0, biggest_f;
float x_midpoint, current_lcb, current_ucb, mean, median_value, median, mode;
double relative_F;
...
//=====================================================================
    ...
    //gets the largest value in the frequency list
    ...
    for (size_t i = 1; i <= k; i++){
        ...
        //================================
        //CENTRAL TENDENCIES CALCULATION
        //================================
        ...
        if (biggest_f == frequencies[i]){
            mode = current_lcb+((frequencies[i]-frequencies[i+1])/(float)((2*frequencies[i])-frequencies[i+1]-frequencies[i-1]))*i_inp;
        }
    }
    rowline();
